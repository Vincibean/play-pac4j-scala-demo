...
trait SecuritySpec extends PlaySpecification {
  ...
  "The facebook page" should {
    "be secured with the facebook client" in new WithApplication(appBuilder.build()) {
  ...
  "The twitter page" should {
    "be secured with the twitter client" in new WithApplication(appBuilder.build()) {
      checkRedirect(s"$baseUrl/twitter/index.html", "twitter.com")
    }
  }

  "The form protected page" should {
    "be secured and redirects to the loginForm" in new WithApplication(appBuilder.build()) {
...
//  }
  ...
  "The basic auth protected page" should {
    "be secured and redirect to callback url for basic auth" in new WithApplication(appBuilder.build()) {
      checkRedirect(s"$baseUrl/basicauth/index.html", "callback")
    }
  }

  "The protected page" should {
    "be secured" in new WithApplication(appBuilder.build()) {
      checkUnAuthorized(s"$baseUrl/protected/index.html")
    }
    "return ok when authorized" in new WithApplication(appBuilder.build()) {
  ...
  "The protected custom page" should {
    ...
    "return forbidden when not custom authorized" in new WithApplication(appBuilder.build()) {
    ...
    "return ok when custom authorized" in new WithApplication(appBuilder.build()) {
